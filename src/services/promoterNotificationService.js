// services/promoterNotificationService.js
//
// Promoter-facing notifications (new referral, welcome). Delegates transport
// to mailerService so credentials + sender identity are centralized.
import { Promoter, PromoCodeRedemption } from '../models/promoterModel.js';
import { sendEmail } from './mailerService.js';

// Strip HTML for the text/fallback body.
...
export const notifyPromoterOfReferral = async (promoter, newCustomer, redemptionId) => {
  try {
    ...
    const subject = `New referral: Someone used your code ${promoter.referral_code}!`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="color: #6b7280; font-size: 14px; margin-top: 30px;">- The Intervos Team</p>
      </div>
    `;

    await sendEmail(promoter.promoter_email, subject, htmlToText(html), html);
...
export const sendPromoterWelcomeEmail = async (promoter) => {
  try {
    const subject = 'Welcome to the Intervos Promoter Program!';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
